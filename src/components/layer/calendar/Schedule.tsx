...
import { getDiffTimeMinutes, getTimeZoneList, isCheckTimeInRange, transferTimeZoneToSettingLessonTime } from '@utils/date';
...
const ModalSchedule = ({ ...props }: Props) =>{
  ...
  // TODO any
  const updateLessonTime = useCallback((index: number, item: any, isAttendance: boolean) => {
  ...
  customerLessonList.length > 0 && customerLessonList.map((item) => {

    const lessonStartTime = item.start_time;
    const lessonEndTime = item.end_time;
    const diffTimeMinutes = getDiffTimeMinutes(lessonStartTime,lessonEndTime);
    const isSameLessonSettingTime = (diffTimeMinutes % lesson_setting_time) === 0;

    for(let index = 0; index < lessonTimeList.length; index++){

      const { startTime, endTime } = lessonTimeList[index];

      const isCheckStartTime = isCheckTimeInRange(startTime, endTime, lessonStartTime);
      const isCheckEndTime = isCheckTimeInRange(startTime, endTime, lessonEndTime);
      const isAttendance = (item.coach_attendance || item.customer_attendance) ? true : false;

      // 센터에서 설정한 시간과 일치하는 단위일 경우와, 강제로 주입한 경우
      if((item.is_forced_lesson_change === FORCE_LESSON_CHANGE && isSameLessonSettingTime && isCheckStartTime && isCheckEndTime) ||
        (!isSameLessonSettingTime && isCheckStartTime && (lessonStartTime !== endTime))){
  ...
  return(
    <Container>
      <NormalList.UnOrderList css={{ height: '100%', margin: '12px 12px 0 0' }}>
        {lessonTimeList.map((item: any) => { // TODO Any 삭제
          ...
          return(
            <NormalList
              ...
              css={{
                ...
                backgroundColor: customerInfo?.id && (customerInfo?.id === item.id ? 'var(--grey500)' : ''),
              ...
              onClick={() => { item.id && setCustomerInfo(item) }}
            >
              {item.isAttendance !== undefined ?
                <>
                  {(() => {
                    if(item.isAttendance){
                      return <Status css={{ width: '12%', textAlign: 'center', backgroundColor: 'var(--blue900)', color: 'var(--blue100)'}}>출석</Status>
                    }
                    ...
                    if(!item.isAttendance){
                      ...
                      if(isAfterNow){
                        return <Status css={{ width: '12%', textAlign: 'center', backgroundColor: 'var(--green900)', color: 'var(--green150)'}}>예약</Status>
                      }
                      else{
                        return <Status css={{ width: '12%', textAlign: 'center', backgroundColor: 'var(--pink900)', color: 'var(--pink100)'}}>결석</Status>
                      }
                    }

                    // TODO 현재는 ID로만 가지고 오고 있어서, 나중에 이름으로 가져오기
                  })()}
                  <div css={{ width: '29%', textAlign: 'center' }}>{`${item.startTime} - ${item.endTime}`}</div>
                  <div css={{ width: '10%', textAlign: 'center' }}>개인</div>
                  <div css={{ width: '15%', textAlign: 'center' }}>3번 코트</div>
                  <div css={{ width: '20%', textAlign: 'center' }}>루카스</div>
                  <div css={{ width: '17%', textAlign: 'center' }}>광개토대왕</div>
                </>
                :
                <>
                  <Status css={{ width: '12%', textAlign: 'center' }}>미예약</Status>
                  <div css={{ width: '29%', textAlign: 'center' }}>{`${item.startTime} - ${item.endTime}`}</div>
...
const Status = styled.div<CSS_TYPE>(
  ...
  (props) => ({
    backgroundColor: props.backgroundColor ? props.backgroundColor : 'var(--grey1000)',
