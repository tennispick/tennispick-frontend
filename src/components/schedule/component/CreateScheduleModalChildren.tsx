...
import { createScheduleModalInputList } from '../data/createScheduleModal';
...
const CreateScheduleModalChildren = () => {
  ...
  const { scheduleType } = formData;

  const getInputComponentByType = (type: string) => {
    if (type === 'scheduleType')
      return (
        <ScheduleTypeInputRadioContainer onChangeFormData={onChangeFormData} />
      );
    if (type === 'customer')
      return (
