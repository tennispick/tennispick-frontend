...
> => await authAxiosInstance.get(`${URL_COACH_LESSON_LIST}`);

export const getCoachDetail = async (
  coachId: string,
): Promise<Response<CoachDetailData>> =>
  await authAxiosInstance.get(`${URL_COACH}/${coachId}`);

export const createCoach = async (params: FormData) => {
  try {
    const result = await authAxiosInstance.post(`${URL_COACH}`, params, {
...
export const deleteCoach = async (params: CoachDeleteApiPayload) =>
  await authAxiosInstance.delete(`${URL_COACH}/${params.coachId}`);
...
export const getCoachCustomers = async (
  ...
  const { coachId, ...rest } = params;
  return await authAxiosInstance.get(URL_COACH_CUSTOMERS(coachId), {
    params: { ...rest },
  });
};

export const updateCoachDetail = async (coachId: string, params: FormData) => {
  return await authAxiosInstance.put(`${URL_COACH_DETAIL(coachId)}`, params, {
    headers: { 'Content-Type': 'multipart/form-data' },
  });
};
