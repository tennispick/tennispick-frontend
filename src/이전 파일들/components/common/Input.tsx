...
const CONTAINER_VARIANT_STYLE = {
  ...
  labelBox:
    'relative mb-6 mx-auto border border-grey100 rounded px-4 pt-3 pb-2',
  file: 'w-full h-full',
};
...
const INPUT_TEXTFIELD_VARIANT_STYLE = {
  default:
    'relative w-full h-full py-1 pl-3 font-normal border border-grey300 rounded-lg outline-none z-[1]',
  labelBox:
    'relative w-full h-full px-0 pl-3 font-normal border-none outline-none z-[1]',
  file: 'display-none',
};
  ...
  (
    ...
    return (
      <>
        ...
        />
        {requiredStatus && (
          <div className="mt-2 ml-1 text-red200 font-bold">{requiredText}</div>
        )}
        {isRegexCheck && (
          <div className="mt-2 ml-1 text-red200 font-bold">{regexText}</div>
        )}
      </>
    );
  },
