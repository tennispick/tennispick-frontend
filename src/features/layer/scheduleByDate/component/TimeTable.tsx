...
import { getDiffTimeMinutes, getTimeZoneList, transferTimeZoneToSettingLessonTime } from "@utils/date";
import { useRecoilState } from "recoil";
import { LessonTimeType } from "../type/lesson";
...
const ScheduleByDateTimeTable = ({ day }: Props) =>{
  ...
  const { data, isFetching, isLoading } = useScheduleByDateQuery({ day });

  const handleAttendanceCheck = (isAttendance: boolean, isPassNowTime: boolean) =>{
    if(isPassNowTime){
      return isAttendance ? <Status backgroundColor={'var(--green100)'} >출석</Status> : <Status backgroundColor={'var(--red100)'} >결석</Status>
    }
    else{
      return isAttendance ? <Status backgroundColor={'var(--green100)'} >출석</Status> : <Status backgroundColor={'var(--grey100)'} >강습전</Status>
    }
  }
  ...
  return(
    <div css={{
      ...
      <Li.UnOrderList css={{ height: 'calc(100% - 60px)', margin: '12px 0 0 0', padding: '0 12px 0 12px' }}>
        {data?.map(({ id, coachAttendance, customerAttendance, startTime, endTime, originStartTime, originEndTime, lessonType, coachName, customerName }: any) => {
          
          const isAttendance = (coachAttendance || customerAttendance) ? true : false;
