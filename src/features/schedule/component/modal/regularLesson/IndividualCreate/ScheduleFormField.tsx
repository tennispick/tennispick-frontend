...
import { FormIndividualCreateType, FormIndividualHandlerType } from "@features/schedule/type/schedule.type";
...
const ScheduleModalRegularLessonIndividualCreateScheduleFormField = ({ scheduleType, lesson, formData, onChangeFormData, setFormData}: Props) =>{

  const { data: courtList } = useGetCourtList();
  const { data: coachList } = useGetCoachList();

  const handleFormFieldChange = (index: number, e: ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    const { name, value } = e.target;
    setFormData((prev) => prev.map((item, idx) => {
      if(idx === index){
        return {
          ...item,
          [name]: value
      ...
      return(
        <div
          ...
          <div css={{ position: 'relative', width: '100%' }}>
            <InputContainer>
              <ScheduleModalRegularLessonIndividualCreateScheduleFormField.LessonDateType index={index} handleFormFieldChange={handleFormFieldChange} />
              <ScheduleModalRegularLessonIndividualCreateScheduleFormField.LessonTime index={index} handleFormFieldChange={handleFormFieldChange} />
              <ScheduleModalRegularLessonIndividualCreateScheduleFormField.WeeklyLessonCount index={index} handleFormFieldChange={handleFormFieldChange} />
...
ScheduleModalRegularLessonIndividualCreateScheduleFormField.LessonDateType = ({ index, handleFormFieldChange }: FormIndividualHandlerType) => {
  return(
    <Select
      name={'lessonDateType'}
      width={'12%'}
...
ScheduleModalRegularLessonIndividualCreateScheduleFormField.LessonTime = ({ index, handleFormFieldChange }: FormIndividualHandlerType) => {
  return(
    <Select
      name={'lessonTime'}
      width={'12%'}
...
ScheduleModalRegularLessonIndividualCreateScheduleFormField.WeeklyLessonCount = ({ index, handleFormFieldChange }: FormIndividualHandlerType) => {
  return(
    <Select
...
ScheduleModalRegularLessonIndividualCreateScheduleFormField.Coach = ({
  ...
  coachList
}: FormIndividualHandlerType & {coachList: Array<{[key:string]: string }>}) => {
  return(
    <Select
      name={'coach'}
      width={'12%'}
...
ScheduleModalRegularLessonIndividualCreateScheduleFormField.Court = ({
  ...
  handleFormFieldChange,
  courtList,
}: FormIndividualHandlerType & {courtList: Array<{[key:string]: string }>}) => {
  return(
    <Select
      name={'court'}
      width={'12%'}
...
ScheduleModalRegularLessonIndividualCreateScheduleFormField.Schedule = ({
  ...
  return(
    <>
      ...
      <div css={{ position: 'relative', width: '25%', display: 'flex', alignItems: 'center' }}>
        <TimeRange
          index={index}
          scheduleType={scheduleType}
