...
const ScheduleModalSearchInput = ({
  ...
  const onClickSaveCustomerLessonHistoryHandler = (target: CustomerLessonHistoryData['lessonHistory'][]) =>{
    ...
    const { lessonId, coachId, courtId, lessonDateType, date, day, startTime, endTime } = target[0];

    const isAvailableLessonItem = lessonList?.find((item) => item.lessonId === lessonId);
    const lessonItemId = isAvailableLessonItem ? isAvailableLessonItem.lessonId.toString() : lessonList![0].id.toString();

    const isAvailableCoachItem = coachList?.find((item) => item.id === coachId);
    const coachItemId = isAvailableCoachItem ? isAvailableCoachItem.id.toString() : coachList![0].id.toString();

    const isAvailableCourtItem = courtList?.find((item) => item.id === courtId);
    const courtItemId = isAvailableCourtItem ? isAvailableCourtItem.id.toString() : courtList![0].id.toString();
