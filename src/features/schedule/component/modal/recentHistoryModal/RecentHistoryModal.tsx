...
type Props = {
  ...
  onClickSaveCustomerLessonHistoryHandler: (target: CustomerLessonHistoryData['lessonHistory'][]) => void;
}

const ScheduleModalRecentHistoryModal = ({ customerId, onClickCloseModalHandler, onClickSaveCustomerLessonHistoryHandler }: Props) => {

  const [checkHistoryId, setCheckHistoryId] = useState<string>('');
  const [currentPage, setCurrentPage] = useState<number>(1);

  const { data } = useCustomerLessonHistoryQuery({ customerId: customerId, page: currentPage });
  ...
  const onClickSaveHandler = () => {
    ...
    const target = lessonHistory.filter(({ id }: Pick<CustomerLessonHistoryData['lessonHistory'], 'id'>) => id === Number(checkHistoryId));
