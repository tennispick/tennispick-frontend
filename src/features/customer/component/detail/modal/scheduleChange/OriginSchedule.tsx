...
import { transferLessonDateType, transferLessonType } from "@features/schedule/util/transfer";
import { isEmptyObj } from "@utils/object";

type Props = {
  customerId: number;
  customerLessonId: number;
  onClickSelectOriginScheduleHandler: (item: CustomerLessonScheduleHistoryData) => void;
  selectSchedule: CustomerLessonScheduleHistoryData;
}

const OriginSchedule = ({ customerId, customerLessonId, onClickSelectOriginScheduleHandler, selectSchedule }: Props) => {
  ...
  return (
    <section css={{ margin: '0 0 12px 0'}}>
              ...
              return (
                <ManageListRow
                  ...
                  css={{
                    padding: '4px 0',
                    height: '36px',
                    backgroundColor: isSelect ? 'var(--grey200)' : 'var(--white100)',
                    cursor: 'pointer',
                  }}
      ...
      <div css={{ fontSize: '0.9rem' }}>
        ...
        : <div css={{ textAlign: 'center', padding: '12px 0 8px 0' }}>선택된 일정이 없어요.</div>}
      </div>
    </section>
  )
};
