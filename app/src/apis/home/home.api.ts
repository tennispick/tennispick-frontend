...
export const getCustomerStatistics = async (date: Date): Promise<AxiosResponse<{ data: CustomerStatistics }>> =>
  await axios.get(`${URL_HOME_CUSTOMER_STATISTICS}?date=${date}`);

export const getSalesStatistics = async (date: Date): Promise<AxiosResponse<{ data: any }>> =>
